...
from .phase10_experiment import build_phase10_experiment_display_report
...
from .sandbox_evaluation import (
    UnknownSandboxEvaluationScenarioError,
...
class AgentRequestHandler(BaseHTTPRequestHandler):
    def do_GET(self) -> None:
        ...
        if path == "/phase10-global-status/demo":
            self._handle_phase10_global_status_demo()
            return

        if path == "/phase10-experiment/demo":
            self._handle_phase10_experiment_demo()
            return
    ...
    def _handle_sandbox_evaluation_demo(self) -> None:
        try:
            query = parse_qs(urlparse(self.path).query)
            scenario_id = _first_query_value(query, "scenario_id") or _first_query_value(query, "name")
            payload = evaluate_sandbox_experiment_scenarios(
    ...
    def _handle_phase10_global_status_demo(self) -> None:
        self._send_json(build_phase10_global_status_report())

    def _handle_phase10_experiment_demo(self) -> None:
        self._send_json(build_phase10_experiment_display_report())
